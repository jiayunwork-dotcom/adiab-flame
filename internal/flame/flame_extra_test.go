...
func TestRichUnburnedFuel(t *testing.T) {
	// At phi=1.2 the oxygen is insufficient to burn all fuel, so the
	// product stream must carry the unburned fraction 1 - 1/phi.
	s := newTestSolver()
...
func TestDissociationFormsCO(t *testing.T) {
	// With the optional equilibrium enabled the product stream must contain
	// CO and H2, and their fractions must be positive and small.
	s := newTestSolver()
...
func TestDissociationLowersTad(t *testing.T) {
	// Endothermic dissociation consumes part of the heat release, so the
	// equilibrium-refined temperature must sit below the simple model.
	s := newTestSolver()
...
func TestEthaneStoichTad(t *testing.T) {
	// The C2 fuel must also close the enthalpy balance and land in a
	// plausible temperature window.
	s := newTestSolver()
...
func TestCrossChecksAllPass(t *testing.T) {
	// The pinned cross rules (phi ordering, inlet trend, conservation) must
	// all hold for the built-in fuels.
	s := newTestSolver()
...
func TestConfigFromJSONValidation(t *testing.T) {
	// Unknown JSON fields must be rejected so a typo cannot silently change
	// the calculation.
