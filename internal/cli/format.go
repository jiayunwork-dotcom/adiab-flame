...
// FormatResult renders the solver output: flame temperature, product mole
// fractions and the atom residuals. Every number comes from the solve, so
// the printed values are exactly the ones tests assert on.
...
// displayNames returns the product species in a stable display order,
// appending the fuel name when unburned fuel is present.
...
// FormatCompact renders a one-line summary used by scripts.
